...
def gen_dico_vide(freqalpha,alpha):
    ...
    for i in range(len(alpha)):
         freqalpha[alpha[i]] = 0
    print(freqalpha)
##############################Version Avancée élève 3 ##################################
def gen_2dico_vide(proba2alpha,alpha):
    """générer un dictionnaire vide associant toutes les lettres présentes dans 
        la liste alpha avec toutes les lettres présentes dans  la liste alpha 
        avec la valeur 0 
        :param proba2alpha: dictionnaire vide en entrée modifié dans la fonction
                            associant une lettre de alpha avec toute lettres de alpha 
                            avec la valeur 0                 
        :type proba2alpha: dictionnaire  
        :param alpha: la liste des lettres de l'alphabet
        :type alpha: liste                
    """  
    for i in range(len(alpha)):
         ...
         for j in range(len(alpha)):
             sousDico[alpha[j]] = 0 
         proba2alpha[alpha[i]] = sousDico
#    print(proba2alpha)
         
def calcul_proba2D_lettre(proba2alpha,nomfichier):
    """ compléter le dictionnaire vide de probabilité d'une lettre associée avec une autre lettre
        en utilisant un fichier de mots pour la probabilité
        :param nomfichier: le nom du fichier à lire
        :type nomfichier:  String
        :param proba2alpha: dictionnaire vide en entrée modifié dans la fonction
                            associant une lettre de alpha avec toute lettres de alpha 
                            avec sa probabilité d'apparition associée avec une autre lettre
                            dans un mot
        :type proba2alpha: dictionnaire                 
    """      
    ...
    if os.path.isfile(nomfichier): # retourne vrai si le fichier existe
        ...
        while mot !="" : 
            ...
            for l in unique:
                for l2 in unique:
                    if (l!=l2) and l in proba2alpha.keys() and l2 in proba2alpha[l].keys():
                        proba2alpha[l][l2] = proba2alpha[l][l2]+1
                    
#            print(proba2alpha)     
        print("nombre de mots",nbm)
        fichier.close()   
        if nbm>0:
            for l in proba2alpha.keys():
                for l2 in proba2alpha[l].keys():
                         proba2alpha[l][l2] = proba2alpha[l][l2]/nbm
#            print(proba2alpha)    
##############################Version Avancée élève 1 ##################################

def demo_v4(lettres, lderniere, proba2alpha ):
    """ Trouver la lettre la plus probable associée avec la dernière lettre trouvée 
         qui n'a pas déjà été proposée.   
        :type lettres: liste 
        :param lderniere: la dernière lettre trouvée
        :type lderniere: string 
        :param proba2alpha: dictionnaire associant une lettre de alpha avec toute lettres de alpha 
                            avec sa probabilité d'apparition associée avec une autre lettre
                            dans un mot
        :type proba2alpha: dictionnaire      
        :return l:  la lettre 
    ...
    for clef, valeur in proba2alpha[lderniere].items():                   
        Trouve=False
...
def demo_v5(lettres, devine, proba2alpha,probaalpha ):
    """ Trouver la lettre la plus probable associée avec les lettres déjà trouvées 
         qui n'a pas déjà été proposée.   
        :type lettres: liste 
        :param devine: liste des lettres déjà trouvées ou des tirets 
        :type devine:  list    
        :type devine: string 
        :param proba2alpha: dictionnaire associant une lettre de alpha avec toute lettres de alpha 
                            avec sa probabilité d'apparition associée avec une autre lettre
    ...
    max2L=""
    sommealpha={}
    sommealpha = probaalpha.copy()
#    gen_dico_vide(sommealpha,alpha)
    for l in devine:
        if l != '-' :            
            for clef, valeur in proba2alpha[l].items():
                sommealpha[clef] = sommealpha[clef] +  valeur                             
    for clef, valeur in sommealpha.items():                    
        Trouve=False
        for i in range(len(lettres)):
            if clef == lettres[i]:
                Trouve=True
        if (max2F < valeur) and (Trouve == False):
            max2F=valeur
            max2L=clef                        
    return max2L
...
def prog_principal(ihm,probaalpha):
    ...
    proba2alpha ={}
    print(probaalpha)
    gen_2dico_vide(proba2alpha,alpha)
    calcul_proba2D_lettre(proba2alpha,"liste_francaisU.txt");    
    print(proba2alpha)  
    ...
    while mode != 3 :       # boucle pour rejouer  
        ...
        lettrederniere=""
#        motcache="riches" 
#        motcache="liquide"  # "marques"   
#        motcache="honteux"  #"chlore"
        ...
        while (Trouve == False) and (nberreur <ERREURMAX):  # boucle par proposition de lettre 
           ...
           #partie 1
            if lettrederniere == "" :
                ...
                print("lettre proposée ",lettre)
            else :
                 lettre = demo_v4(lettresP,lettrederniere, proba2alpha) 
                 print("lettre proposée ",lettre)
                 print("Lettre qui aurait été proposée "+demo_v2(lettresP, probaalpha ))
            lettresP.append(lettre[0])
            if(lettre_presente(motcache,lettre)==True):
                devoiler_lettre(motcache,devine,lettre)
                lettrederniere=lettre#demov4
                dessin_texte(stylo,devine,placement[0])
